...
def generalSetupPage() {
	...
	dynamicPage(name: "generalSetupPage", uninstall: true, nextPage: roomsSetupPage) {
		...
		section("Close the vent totally [optional, default=some safeguards are implemented to avoid damaging your HVAC]") {
...
def roomsSetup(params) {
	...
	dynamicPage(name: "roomsSetup", title: "Rooms Setup", uninstall: true, nextPage: zonesSetupPage) {
		...
		section("Room ${indiceRoom}-Do temp/vent adjustment when occupied room only [optional, vent will be partially closed otherwise]") {
...
/*		Y.R. Commented out as there is an issue with Schedule page for this parameter (not distinct)        
		section("Schedule ${indiceSchedule}-Minimum Fan Time during the Schedule [optional]") {
			input (name: "givenFanMinTime${indiceSchedule}", "text", title: "Minimum fan runtime for this schedule",
...
*/        
		...
		section("Schedule ${indiceSchedule}-Adjust vent settings every 5 minutes during the schedule [optional, default=false]") {
...
/* Y.R. Commented out as the schedule parameter not distinct per schedule as excpected.
...
*/                
				...
				if (foundVentSwitch ==null) {
					...
					def ventLevel= ventSwitch.getLevel()
					if ((ventLevel!=null) && (ventLevel < MIN_OPEN_LEVEL)) { // below minimum level is considered as closed.
						nbClosedVents++ 
						closedVentsSet.add(ventSwitch)                        
						log.debug("turn_off_all_other_vents>${ventSwitch}'s level=${ventLevel} is lesser than minimum level ${MAX_OPEN_LEVEL}")
  					}                        
